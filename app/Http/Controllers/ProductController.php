...
class ProductController extends Controller
{
    ...
    public function product_details($slug)
    {
    	...
    	$pro = DB::table('product')->join('imageproduct','product.id_pro','imageproduct.id_pro')->join('cate_product','product.cate_parent','cate_product.id_cate')->where('slug_pro','=',$slug)->groupBy('product.id_pro')->first();
    	...
    	$ref_pro = DB::table('product')->join('imageproduct','product.id_pro','imageproduct.id_pro')->where('cate_parent',$pro->cate_parent)->where('product.id_pro','!=',$pro->id_pro)->groupBy('product.id_pro')->take(6)->get();
    	$arr = [
    		'pro'=>$pro,
    		'imgs'=>$img,
    		'ref_pro'=>$ref_pro
